...
SCHEMA_FIELDS = ["postal_code", "chome", "gender", "age_group", "newspaper", "registered_at"]
...
def fetch_customers(
    ...
    for raw in rows[1:]:
        ...
        # 郵便番号が無い行は無効データとして除外
        if len(rec["postal_code"]) != 7:
            logger.info("郵便番号が不正なためスキップ: %r", rec.get("postal_code"))
            continue
